...
def _import_mcp_sdk():
    ...
    异常：
    - RuntimeError：MCP SDK 无法加载时抛出，并附带安装建议。
    """
    try:
        ...
        raise RuntimeError(
            "MCP SDK 无法加载（常见原因：未安装 `mcp`、版本过旧缺少 "
            "`streamable_http`、或缺少 `httpx`）。"
            f" 原始错误：{exc!s}。"
            " 建议执行：`python -m pip install -U \"mcp>=1.12\" httpx`。"
        ) from exc
...
class StreamableHttpMcpClient:
    ...
    async def _with_session(self, action, *args):
        """功能：建立 MCP HTTP 会话并在其中执行指定异步操作。
        参数：
        - action：接收 ClientSession 的异步回调函数。
        - *args：传递给 action 的额外位置参数。
        返回值：
        - Any：action 的返回值。
        """
    ...
    async def _list_tools(self, session) -> List[Dict[str, Any]]:
        """功能：在已建立的 MCP 会话中拉取工具列表并规范化字段。
        参数：
        - session：已初始化的 ClientSession 实例。
        返回值：
        - List[Dict[str, Any]]：含 name、description、input_schema 的工具字典列表。
        """
    ...
    async def _call_tool(self, session, tool_name: str, arguments: Dict[str, Any]) -> str:
        """功能：在已建立的 MCP 会话中调用工具并提取文本结果。
        参数：
        - session：已初始化的 ClientSession 实例。
        - tool_name：远端工具名称。
        - arguments：调用参数字典。
        返回值：
        - str：工具返回文本或结构化内容的 JSON 字符串。
        异常：
        - RuntimeError：工具返回 is_error 时抛出。
        """
